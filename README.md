...
渐进式披露型知识手册把解释层从阅读流里拆出来，做成**按需展开的抽屉**：

```
遇到不懂的词 → 点它 → 抽屉里看定义和相关概念 → 关掉 → 接着读
```

这个形态不是新发明：游戏圈叫它「P社手册」（Paradox 的 nested tooltips），口语里也叫「点读手册」，学名**渐进披露**（progressive disclosure）。教育技术研究中「纯非线性超文本让读者迷航」的老问题（lost in hyperspace），靠它的**双结构**解决——线性章节主线防迷航，术语抽屉和概念图谱管深度。
